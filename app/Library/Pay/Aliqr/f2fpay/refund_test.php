<?php
header('Content-type: text/html; charset=utf-8'); require_once 'model/builder/AlipayTradeRefundContentBuilder.php'; require_once 'service/AlipayTradeService.php'; if (!empty($_POST['out_trade_no']) && trim($_POST['out_trade_no']) != '') { $sp30c318 = trim($_POST['out_trade_no']); $sp4db26d = trim($_POST['refund_amount']); $sp9810b0 = trim($_POST['out_request_no']); $sp3efb2d = ''; $sp7cb6cf = new AlipayTradeRefundContentBuilder(); $sp7cb6cf->setOutTradeNo($sp30c318); $sp7cb6cf->setRefundAmount($sp4db26d); $sp7cb6cf->setOutRequestNo($sp9810b0); $sp7cb6cf->setAppAuthToken($sp3efb2d); $sp7b93f6 = new AlipayTradeService($spc6bf93); $spfb3ac2 = $sp7b93f6->refund($sp7cb6cf); switch ($spfb3ac2->getTradeStatus()) { case 'SUCCESS': echo '支付宝退款成功:' . '<br>--------------------------<br>'; print_r($spfb3ac2->getResponse()); break; case 'FAILED': echo '支付宝退款失败!!!' . '<br>--------------------------<br>'; if (!empty($spfb3ac2->getResponse())) { print_r($spfb3ac2->getResponse()); } break; case 'UNKNOWN': echo '系统异常，订单状态未知!!!' . '<br>--------------------------<br>'; if (!empty($spfb3ac2->getResponse())) { print_r($spfb3ac2->getResponse()); } break; default: echo '不支持的交易状态，交易返回异常!!!'; break; } return; } ?>
